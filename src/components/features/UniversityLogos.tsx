...
function UniversityLogos() {
  ...
  return (
    <div className="w-full overflow-hidden py-4 relative flex items-center bg-transparent my-1 [mask-image:linear-gradient(to_right,transparent,black_15%,black_85%,transparent)]">
      {/* Fallback gradient masks for older browsers */}
      <div className="absolute left-0 top-0 bottom-0 w-16 bg-gradient-to-r from-background to-transparent z-10 pointer-events-none sm:hidden"></div>
      <div className="absolute right-0 top-0 bottom-0 w-16 bg-gradient-to-l from-background to-transparent z-10 pointer-events-none sm:hidden"></div>
      ...
      <div className="flex w-max animate-marquee hover:[animation-play-state:paused]">
        {marqueeItems.map((logo, index) => {
          ...
          return (
            <div 
              key={`${logo.name}-${index}`} 
              className={`flex items-center justify-center gap-2 px-8 opacity-70 hover:opacity-100 transition-all duration-300 hover:scale-105 cursor-default group`}
            >
              <div className={`p-2 rounded-xl bg-transparent group-hover:bg-card/50 ${logo.color} transition-all duration-300`}>
